...
sepscatter meantax3 year if year<=2016, separate(Region_Code1) recast(connect) missing  title("Tax Revenue incl. SC Collection: `w' vs Regions")
graph export "charts/`w' Tax Revenue and Social Contributions vs Regions.png", replace

sort Region_Code year
sepscatter meantax6 year if year<=2016, separate(Region_Code1) recast(connect) missing  title("Tax Revenue Collection: `w' vs Regions")
...
twoway (connected meantax3 year if year<=2016 & Region_Code1=="`x'", msymbol(Oh))(connected Tax_Revenue_incl_SC year if year<=2016 & Country_Code=="`v'"), legend(lab(1 "`x': Tax Revenue and SC") lab(2 "`w': Tax Revenue and SC")) ytitle("% of GDP") title("`w' vs. `x' - Comparing Tax Collection")
...
title("`w' versus neighbours: Tax Revenue incl. SC over time")) ///
...
twoway (connected Tax_Revenue year if Country_Code=="`v'", msymbol(O) ///
legend(label(1 "`w'") label(2 "`a1'") label(3 "`b1'") label(4 "`c1'") ///
label(5 "`d1'")) ytitle("% of GDP") ///
title("`w' versus neighbours: Tax Revenue over time")) ///
...
graph bar meantaxinc2  meantaxvat2 meantaxexcises2 meantaxtrade2 meantaxother_tax2 meantaxpropr2 if (year==2015 & Region_Code2=="`x'")| (year==2015 & Region_Code2=="`w'"), over(Region_Code2, relabel(1 "`w'" 2 "`x'")) ytitle("% of GDP") title(" Tax Collection by Tax Type (2015) - `w' versus `x'")  legend(label(1 "Income Tax")  label(2 "VAT") label(3 "Excise Tax") label(4 "Trade Taxes") label(5 "Other Taxes") label(6 "Property Tax"))
...
twoway (connected Tax_Revenue_buoyancy year if year<=2016 & Country_Code=="`v'", 
...
#delimit ;
twoway (connected Income_Taxes_buoyancy year if year<=2016 & Country_Code=="`v'",
...
#delimit ;
twoway (connected Value_Added_Tax_buoyancy year if year<=2016 & Country_Code=="`v'", 
...
#delimit ;
twoway (connected Trade_Taxes_buoyancy year if year<=2016 & Country_Code=="`v'", 
msymbol(X) yline(0) yline(1, lp(dash)) ytitle("Multiples of % change in GDP", size(small))), 
...
twoway (line Tax_Capacity  year if Country_Code=="`v'" & year>=1995) (connected Tax_Revenue_incl_SC  year if Country_Code=="`v'"), legend(lab(1 "Tax Capacity") lab(2 "Tax Revenue incl. SC")) ytitle("% of GDP") title("`w': Tax Capacity and Performance")
graph export "charts/`w' Tax Capacity.png", replace

twoway (line Tax_Effort  year if Country_Code=="`v'" & year>=1995), title("`w': Tax Effort")
graph export "charts/`w' Tax Effort.png", replace

twoway (line Tax_Gap  year if Country_Code=="`v'" & year>=1995), title("`w': Tax Gap")
