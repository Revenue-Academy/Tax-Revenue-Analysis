...
def is_downward_trend(df, start_year, end_year, target_year, tax_type):
    tol = (df[df['year']==target_year][tax_type].values[0])/250
    downward_trend = False
    if ((target_year >= start_year+1) and (target_year <= end_year-1)):
        if ((df[df['year']==target_year-1][tax_type].values[0] >
            df[df['year']==target_year][tax_type].values[0] + tol) and
            (df[df['year']==target_year][tax_type].values[0] >
            df[df['year']==target_year+1][tax_type].values[0] + tol)):
            downward_trend = True    
    return downward_trend

def is_upward_trend(df, start_year, end_year, target_year, tax_type):
    tol = (df[df['year']==target_year][tax_type].values[0])/250
    upward_trend = False
    if ((target_year >= start_year+1) and (target_year <= end_year-1)):
        if ((df[df['year']==target_year+1][tax_type].values[0] >
...
# Simple V Shape
def is_v_shaped(df, start_year, end_year, target_year, tax_type):
    v_shaped = False
    if ((target_year >= start_year+1) and (target_year <= end_year-1)):
        if ((df[df['year']==target_year-1][tax_type].values[0] >
            df[df['year']==target_year][tax_type].values[0]) and
            (df[df['year']==target_year][tax_type].values[0] <
            df[df['year']==target_year+1][tax_type].values[0])):
            v_shaped = True    
    return v_shaped

# V shape with two years of recovery
def is_v_shaped_long(df, start_year, end_year, target_year, tax_type):
    v_shaped = False
    if ((target_year >= start_year+1) and (target_year <= end_year-2)):
        if ((df[df['year']==target_year-1][tax_type].values[0] >
            df[df['year']==target_year][tax_type].values[0]) and
            (df[df['year']==target_year][tax_type].values[0] <
            df[df['year']==target_year+1][tax_type].values[0]) and
            (df[df['year']==target_year+1][tax_type].values[0] <
            df[df['year']==target_year+2][tax_type].values[0])):
            v_shaped = True    
    return v_shaped

# Simple V Shape
def is_reverse_v_shaped(df, start_year, end_year, target_year, tax_type):
    reverse_v_shaped = False
    if ((target_year >= start_year+1) and (target_year <= end_year-1)):
        if ((df[df['year']==target_year-1][tax_type].values[0] <
            df[df['year']==target_year][tax_type].values[0]) and
            (df[df['year']==target_year][tax_type].values[0] >
            df[df['year']==target_year+1][tax_type].values[0])):
            reverse_v_shaped = True  
    return reverse_v_shaped

def pre_crisis_yr(df, start_year, end_year, crisis_year, tax_type):
    pre_crisis_year = crisis_year-1
    # makes sure we have enough values to do the comparison
    if is_v_shaped(df, start_year, end_year, crisis_year, tax_type):
        pre_crisis_year = crisis_year - 1
    elif is_reverse_v_shaped(df, start_year, end_year, crisis_year, tax_type):
        pre_crisis_year = crisis_year
    elif (is_downward_trend(df, start_year, end_year, crisis_year, tax_type) and
          is_v_shaped(df, start_year, end_year, crisis_year+1, tax_type)):
        pre_crisis_year = crisis_year-1    
    elif (is_upward_trend(df, start_year, end_year, crisis_year, tax_type) and
          is_reverse_v_shaped(df, start_year, end_year, crisis_year+1, tax_type)):
        pre_crisis_year = crisis_year+1      
    ...
    if (crisis_year <= end_year-2):
        if ((df[df['year']==crisis_year+1][tax_type].values[0] >
            ...
            return(crisis_year + 1)
    """
    print('pre crisis year: ',pre_crisis_year)
    return(pre_crisis_year)            
    
def find_pre_crisis_trend(df, start_year, end_year, pre_crisis_year, tax_type):
    #adjusted crisis year is sent into the function    
    #tol = 0.0
    #print("pre_crisis_year: ", pre_crisis_year)
    #print(df[df['year']==pre_crisis_year][tax_type].values[0])    
    #print("tax_type: ", tax_type)
    tol = (df[df['year']==pre_crisis_year][tax_type].values[0])/250
    print('tol: ', tol)
    pre_crisis_trend = ""
    print(' pre_crisis year-1: ', df[df['year']==pre_crisis_year-1][tax_type].values[0])
    print(' pre_crisis year: ',df[df['year']==pre_crisis_year][tax_type].values[0])
    print(' pre_crisis year+1: ',df[df['year']==pre_crisis_year+1][tax_type].values[0])
    print(' pre_crisis year+2: ',df[df['year']==pre_crisis_year+2][tax_type].values[0])
    
    if (pre_crisis_year >= start_year-1) and (pre_crisis_year <= end_year-1):
        if ((df[df['year']==pre_crisis_year-1][tax_type].values[0] -
             df[df['year']==pre_crisis_year][tax_type].values[0]) > tol):
            if ((df[df['year']==pre_crisis_year][tax_type].values[0] -
                df[df['year']==pre_crisis_year+1][tax_type].values[0]) > tol):
                if ((df[df['year']==pre_crisis_year+1][tax_type].values[0] -
                     df[df['year']==pre_crisis_year+2][tax_type].values[0]) > tol):
                    
                    print((df[df['year']==pre_crisis_year+1][tax_type].values[0] -
                     df[df['year']==pre_crisis_year+2][tax_type].values[0]))
        ...
        if ((df[df['year']==pre_crisis_year][tax_type].values[0] -
            ...
            if ((df[df['year']==pre_crisis_year+1][tax_type].values[0] -
                 df[df['year']==pre_crisis_year][tax_type].values[0]) > tol):
                if ((df[df['year']==pre_crisis_year+2][tax_type].values[0] -
                     df[df['year']==pre_crisis_year+1][tax_type].values[0]) > tol):                
                    pre_crisis_trend = "upward"         
    return(pre_crisis_trend)
    
def find_turnaround_year(df, start_year, end_year, pre_crisis_year,
                                        recovery_year, tax_type):
    # look for minimum year around the crisis
    # if there is a change in trend of the parameter
    # for two successive years then that becomes the minimum
    
    minimum_year = np.nan
    pre_crisis_revenue = df[df['year']==pre_crisis_year][tax_type].values[0]
    if (not np.isnan(recovery_year)):
        min_revenue = df[df['year']==pre_crisis_year+1][tax_type].values[0]
        minimum_year = pre_crisis_year+1
        for year in range(pre_crisis_year+1, end_year):
            revenue_year = df[df['year']==year][tax_type].values[0]
            revenue_year_1 = df[df['year']==year+1][tax_type].values[0]
            if (revenue_year<min_revenue):
                min_revenue = revenue_year
                minimum_year = year
                print('minimum year here: ', minimum_year)
                # if there are two years of recovery stop looking further for 
                # minimum year
                if is_upward_trend(df, start_year, end_year, year+1, tax_type):
                    turn_around_year = minimum_year + 1
                    return(minimum_year, turn_around_year)
                # if there is one year of recovery that brings the vlaue 
                # back to pre-criris value then stop looking further for
                # minimum year
                if (is_reverse_v_shaped(df, start_year, end_year, year+1, tax_type) and
                    (revenue_year_1 > pre_crisis_revenue)):
                    turn_around_year = minimum_year + 1
                    return(minimum_year, turn_around_year)                               
    turn_around_year = minimum_year + 1
    return(minimum_year, turn_around_year)

def find_recovery_year(df, end_year, pre_crisis_year, pre_crisis_trend, tax_type):
    ...
    if ((pre_crisis_trend == 'downward') or (pre_crisis_trend == 'upward')):
        ...
        for year in range(pre_crisis_year+1, end_year+1):
            revenue_year = df[df['year']==year][tax_type].values[0]
...
def find_recovery_year_inter(df, end_year, pre_crisis_year, minimum_year, pre_crisis_trend, tax_type):
    ...
    if ((pre_crisis_trend == 'downward') or (pre_crisis_trend == 'upward')):
        ...
        if (not np.isnan(minimum_year)):
            ...
            crisis_revenue = df[df['year']==minimum_year][tax_type].values[0]
            crisis_drop = pre_crisis_revenue - crisis_revenue
            print('pre_crisis_revenue: ', pre_crisis_revenue)
            print('crisis_revenue: ', crisis_revenue)
            for year in range(minimum_year+1, end_year+1):
                revenue_year = df[df['year']==year][tax_type].values[0]
                if np.isnan(revenue_year) and not data_gap:
                    data_gap = True
                print('revenue_year: ', revenue_year)
                #if (pre_crisis_trend != 'upward'):
                if ((revenue_year >= crisis_revenue + 0.5*crisis_drop) and
                ...
                if ((revenue_year >= crisis_revenue + 0.75*crisis_drop) and 
                    ...
                    return(recovery_year_1_2, recovery_year_3_4,
            ...
            if (year == end_year):
                if not(np.isnan(revenue_year)) and not data_gap:
                    ...
                    if np.isnan(recovery_year_3_4):
                        ...
                        print('recovery_year_3_4: ', recovery_year_3_4)
    print(recovery_year_1_2_str, recovery_year_3_4_str)    
    return(recovery_year_1_2, recovery_year_3_4,
           recovery_year_1_2_str, recovery_year_3_4_str)
    
def find_negative_shock(df, pre_crisis_trend, pre_crisis_year, recovery_year, 
                       minimum_year, turn_around_year, param):
    param_shock_value = np.nan
    pre_crisis_param_value = np.nan
    crisis_param_value = np.nan
    pre_crisis_param_value = df[df['year']==pre_crisis_year][param].values[0]
    #print('pre_crisis_trend: ', pre_crisis_trend)
    if ((pre_crisis_trend == 'downward') or
        (pre_crisis_trend == 'upward')):
        param_shock_value = np.nan
    else:
        if ((minimum_year!=0) and not (np.isnan(minimum_year))):
            print('param: ', param)
            print('turn_around_year: ', turn_around_year)
            print('minimum_year: ', minimum_year)
            crisis_param_value = df[df['year']==minimum_year][param].values[0]
